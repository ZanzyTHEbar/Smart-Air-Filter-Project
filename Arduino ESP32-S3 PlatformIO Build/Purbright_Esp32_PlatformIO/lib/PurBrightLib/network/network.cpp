...
void Network::SetupWebServer()
{
    ...
    else
    {
        ...
        if (!PRODUCTION)
        {
            ...
            log_d("[INFO]: This is a test print of the if statement ");
        }
        else
        {
            WiFi.softAP(WIFI_SSID, WIFI_PASS, 10, 0, 3); // AP mode with password
            log_d("[INFO]: This is a test print of the else statement ");
        }

        networkRoutes(); // call the network routes function to setup the routes
        log_d("[INFO]: We have passed the if else conditional block");
    }
}
// ######################## server functions #########################
 ...
 * Description: This function checks the Network connection - is called in the loop() function every 5 seconds
 * Parameters: None
 * Return: None
 ******************************************************************************/
